...
// UDP; ingest.Listener пока его не прокидывает дальше по пайплайну — тот же
// пробел, что и у storage.Event.Source в M4). Вызывающий код передаёт его
// явно, что делает кэш тестируемым независимо от остального пайплайна.
//
