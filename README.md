...
Это капстоун-проект, сделанный, чтобы показать компетенции в Go для роли
SIEM/telemetry-collector: конкурентность под реальной нагрузкой (worker pool, backpressure, graceful shutdown),
разбор бинарных протоколов (темплейт-based state machine у NetFlow/IPFIX), 
подключаемые storage-бэкенды (батч-инсерты в ClickHouse, producer/consumer для Kafka) и,
что важнее всего, — **заявления о производительности, подтверждённые `benchstat`**.
Каждая цифра ниже воспроизводима командой, которая указана рядом.
